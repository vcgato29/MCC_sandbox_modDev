...
if (_playMusic in [0,1]) then {
	_init = format ["0 = _this spawn {if (!isDedicated && playerSide ==  %4) then {waituntil {alive player};player createDiaryRecord ['diary', ['%1',(toString %3) + '%2']];(_this getVariable 'missionsInfo') spawn MCC_fnc_MWopenBriefing;}};",_tittle, _string ,_missionTittle, str _sidePlayer];
